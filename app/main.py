...
def load_db():
    ...
    if "cash_state" not in data:
        data["cash_state"] = DEFAULT_DB["cash_state"]
        changed = True

    if "next_purchase_id" not in data:
        existing_ids = [item.get("id", 0) for item in data.get("purchases", [])]
        data["next_purchase_id"] = (max(existing_ids) + 1) if existing_ids else 1
        changed = True
...
@app.route("/")
def home():
    return jsonify({"ok": True, "app": "Grupo Bolhão API"})
